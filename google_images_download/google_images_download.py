...
import time  # Импорт библиотек для отслеживания времени сборки
import os
import argparse
import ssl
import datetime
...
#получение списка аргументов из командной строки, или из конфигурационного файла
def user_input():
...
class googleimagesdownload:
    ...
    # загрузка содержимого веб-страницы по указанному URL, либо вывод сообщения об ошибке и прекращение работы программы
    ...
    #Загрузка страниц с лимитом более 100
    ...
    # поиск следующего изображения на странице
    ...
    #переделывание объекта в читаемый формат
    ...
    #функция для скачивания одиночного изображения
    ...
    #Создание параметров URL-адреса
    def build_url_parameters(self,arguments):
        ...
        for key, value in params.items():
            if value[0] is not None:
                ...
                # счетчик на то, чтоб узнать добавлен ли первый параметр
                if counter == 0:
    ...
    #создание основного поискового URL-адреса
    ...
    #функция для взятия аргументов из файла
    ...
    def download_image(self,image_url,image_format,main_directory,dir_name,count,print_urls,socket_timeout,prefix,print_size,no_numbering,no_download,save_source,img_src,silent_mode,thumbnail_only,format,ignore_urls):
        ...
        try:
            req = Request(image_url, headers={
                "User-Agent": "Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1312.27 Safari/537.17"})
            try:
                # время ожидания для загрузки изображения
                if socket_timeout:
                    timeout = float(socket_timeout)
                else:
                    timeout = 10
                ...
                #возвращение имени изображения обратно в вызывающий метод, чтобы использовать его для загрузки миниатюр
                download_status = 'success'
    ...
    # нахождение следующего изображения
    ...
    def download_executor(self,arguments):
        ...
        for arg in args_list:
            if arg not in arguments:
                arguments[arg] = None
       #инициализация аргументов от пользователя
        ...
        for pky in prefix_keywords:
            for sky in suffix_keywords:
                ...
                while i < len(search_keyword):
                    ...
                    if limit < 101:
                        raw_html = self.download_page(url)  # загрузка страницы
                    else:
...
def main():
    ...
    for arguments in records:
        ...
        if arguments['single_image']:  # скачать одиночное изображение используя URL
            response = googleimagesdownload()
            response.single_image(arguments['single_image'])
        else:  # загрузка нескольких изображений
            response = googleimagesdownload()
            paths,errors = response.download(arguments)  #wrapping response in a variable just for consistency
            total_errors = total_errors + errors

        t1 = time.time()  # остановка таймера
        total_time = t1 - t0  # подсчет времени, затраченного на поиск и загрузку изображений
        if not arguments["silent_mode"]:
